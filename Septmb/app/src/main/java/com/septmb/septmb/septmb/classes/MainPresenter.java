...
import com.septmb.septmb.septmb.network.API;
...
public class MainPresenter implements MainContract.LoginPresenter {
    ...
    private void send(String email, int amount, String note) {
        ...
        t.setTo(email);
        t.setAmount(Money.of(CurrencyUnit.USD, amount*0.8));
        t.setNotes(note);
        ...
        t2.setTo("cayleywetzig@gmail.com");
        t2.setAmount(Money.of(CurrencyUnit.USD, amount*0.2));
        t2.setNotes("It is this app's processing fee.");
        subscriptions.add(API.getInstance().sendMoney(t2).subscribe(new Consumer<Transaction>() {
            @Override
            public void accept(Transaction transaction) throws Exception {
                view.showMsg("Success");
                view.showProgress(false);
    ...
    private void request(String email, int amount, String note) {
        ...
        Transaction t2 = new Transaction();
        t2.setFrom("cayleywetzig@gmail.com");
        t2.setAmount(Money.of(CurrencyUnit.USD, amount*0.2));
        t2.setNotes("It is this app's processing fee.");
        subscriptions.add(API.getInstance().sendMoney(t2).subscribe(new Consumer<Transaction>() {
            @Override
